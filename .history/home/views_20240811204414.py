...
def enter_password(request, classroom_id):
    ...
    if request.method == 'POST':
        ...
        if entered_password == classroom.password:
            # Thực hiện điều hướng đến trang chi tiết hoặc thao tác khác
            return HttpResponse("Access Granted")  # Thay bằng điều hướng hoặc render trang chi tiết
        else:
            return HttpResponse("Incorrect password")  # Thay bằng thông báo lỗi hiển thị trên modal

    return redirect('classrooms_by_subject_and_grade', subject_id=classroom.subject.id, grade=classroom.grade)
