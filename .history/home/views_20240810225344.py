...
from nextgenedu import settings
...
def authPage(request):
    if request.method == 'POST':
        ...
        if 'google-auth' in request.POST:
            session_token = request.POST.get('clerk-session-token')
            
            # Verify the session token using Clerk's API
            headers = {
                "Authorization": f"Bearer {settings.CLERK_API_KEY}",
                "Content-Type": "application/json"
            }
            session_response = requests.get(
                f"{CLERK_API_URL}/sessions/{session_token}", headers=headers
            )
