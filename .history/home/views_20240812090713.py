...
from home.models import Classroom, CustomUser, Subjects
...
def classroom_detail(request, classroom_id):
    ...
    # Lấy tất cả các comments của classroom (nếu có)
    comments = classroom.comments.all()  # Giả sử bạn có model Comments liên kết với Classroom
    
    classroom_titles = [
        {
            'name': 'Welcome',
            'subtitles': [
                {'name': 'Introduction', 'url': '/classroom/{}/introduction/'.format(classroom.id)},
                {'name': 'Rules', 'url': '/classroom/{}/rules/'.format(classroom.id)},
            ]
        },
        {
            'name': 'Assignments',
            'subtitles': [
                {'name': 'Assignment 1', 'url': '/classroom/{}/assignment/1/'.format(classroom.id)},
                {'name': 'Assignment 2', 'url': '/classroom/{}/assignment/2/'.format(classroom.id)},
            ]
        },
        # Add more titles and subtitles as needed
    ]
    
    context = {
        'classroom': classroom,
        'classroom_titles': classroom_titles,
        'comments': comments,
    }
