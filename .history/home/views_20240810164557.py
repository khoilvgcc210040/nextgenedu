    ...
    if request.method == 'POST':
        ...
        if form_type == 'signup':
            ...
            try:
                ...
                messages.info(request, 'The login details you provided are incorrect. Please try again.', extra_tags='login')
            ...
            if user is not None:
                ...
                messages.info(request, 'Email has not already existed1', extra_tags='login')
                request.session['login_email'] = email
                return redirect('/auth/?form=login')
    else:
        form_type = request.POST.get('form', 'signup')
